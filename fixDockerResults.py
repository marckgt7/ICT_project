...
filtered_blocks = [blocks[i] + blocks[i+1] for i in range(0, len(blocks)-1, 2) if '/app/Secrets1' in blocks[i+1]]
...
# # Mantiene i blocchi che contengono "/app/Secrets" nella sezione "File:"
# filtered_blocks = [blocks[i] + blocks[i+1] for i in range(0, len(blocks)-1, 2) if 'Secrets1' in blocks[i+1]]
