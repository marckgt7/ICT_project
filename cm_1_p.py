...
def analyze_detectors(results_file, additional_fn_detectors):
    ...
    for _, row in df.iterrows():
        ...
        if secrets == 1:
            results[detector]['TP'] += 1  # All 10 are true positives
        elif secrets > 1:
            results[detector]['TP'] += 1  # First 10 are true positives
            results[detector]['FP'] += secrets - 1  # The rest are false positives
        elif secrets < 1:
            results[detector]['TP'] += secrets  # Found secrets are true positives
            results[detector]['FN'] += 1 - secrets  # The rest are false negatives
    ...
    for detector in missing_detectors:
        results[detector]['FN'] += 1  # Entirely missed detector is 10 false negatives
